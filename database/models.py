...
def _utcnow():
    # Despite the historical name, this returns the server's LOCAL naive
    # datetime — the dashboard and reservation date logic assume local time
    # throughout the app (single-locale deployment). Returning UTC here was
    # making "3 minutes ago" alerts display as "3 hours ago" in Lebanon
    # (UTC+3) because the JSON serializer drops the timezone and JS treats
    # the naive value as local. Keep the local-only convention end-to-end.
    return datetime.now()
...
class Guest(Base):
    ...
    # Watchlist — when True, the recognition pipeline fires an
    # AlertType.WANTED alert every time this guest is identified in the
    # live feed. watch_reason is shown to staff in the alert.
    is_watched = Column(Boolean, nullable=False, default=False)
    watch_reason = Column(Text, nullable=True)
    # Staff badge — admin-only flag that suppresses ALL automated alerts
    # for this person (security/assistance/VIP/wanted). Used to silence
    # the system when an off-duty manager or known staff member walks
    # through the lobby. Only StaffRole.ADMIN can toggle this field.
    is_staff_badge = Column(Boolean, nullable=False, default=False)
    staff_badge_label = Column(String(60), nullable=True)  # e.g. "Floor Manager - John"
